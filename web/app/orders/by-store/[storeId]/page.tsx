...
const StoreOrdersPage = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const response = await getMyOrdersByStore(storeId, currentPage, 10);
      ...
      if (error instanceof ApiError) {
        ...
        setErrorMessage("Não foi possível carregar seus pedidos desta loja.");
      }
    } finally {
      setIsLoading(false);
    }
  }, [storeId, getMyOrdersByStore, currentPage]);
          ...
          <div className="mb-16 flex flex-col items-start justify-between gap-6 border-b border-slate-100 pb-12 md:flex-row md:items-end">
            <div>
              ...
              <h1 className="text-6xl font-black uppercase italic tracking-tighter text-slate-950 md:text-7xl">
                Seus <span className="text-indigo-600">Pedidos.</span>
              </h1>
            </div>
          ...
          {orders.length == 0 ? (
            <div className="flex flex-col items-center justify-center rounded-[4rem] border-2 border-dashed border-slate-200 bg-white py-32 text-center">
              ...
              <h2 className="text-2xl font-black uppercase italic tracking-tighter text-slate-950">
                Nenhum pedido{" "}
                <span className="text-slate-400">nesta loja.</span>
              </h2>
                  ...
                  >
                    <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <button className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 transition-all hover:bg-indigo-600 hover:text-white">
