...
**Total geral: 318 testes** (158 service · 93 controller · 7 exception handler · 34 specification · 10 segurança · 16 utilitários).
...
**Total: 93 testes de controller**, cobrindo os seguintes padrões de requisição (não inclui `GlobalExceptionHandlerTest`, documentado na seção seguinte):
...
| `JwtAuthenticationFilter`         | 8      | Sem header e sem cookie → chain continua sem autenticação; Bearer válido + usuário encontrado → autentica no `SecurityContext`; cookie `token` válido + usuário encontrado → autentica; token inválido → chain continua sem autenticação; token válido mas usuário não encontrado no DB → 401 sem chamar chain; header não-Bearer → não valida token; cookies sem nome `token` → não valida token; role do usuário mapeada como `GrantedAuthority`; Bearer tem precedência sobre cookie quando ambos presentes |
