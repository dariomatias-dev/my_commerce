...
- [Testes](#testes)
  ...
  - [GlobalExceptionHandler Testado](#globalexceptionhandler-testado)
  - [Specifications Testadas](#specifications-testadas)
  - [Segurança Testada](#segurança-testada)
  - [Utilitários Testados](#utilitários-testados)
...
O projeto conta com testes de unidade para as camadas de **service**, **controller**, **specification**, **segurança** e **utilitários**, além de testes do `GlobalExceptionHandler`, escritos com **JUnit 5** e **Mockito**. Os testes de service usam `@ExtendWith(MockitoExtension.class)` com injeção via `@InjectMocks` e dependências mockadas com `@Mock`. Os testes de controller usam `@WebMvcTest` para isolar a camada web sem necessidade de banco de dados ou outros serviços externos. Os testes de specification do JPA com dados reais usam `@DataJpaTest` com H2 em memória.

**Total geral: 378 testes** (204 service · 106 controller · 7 exception handler · 34 specification · 11 segurança · 16 utilitários).
...
| `AuthService`             | 17     | `login` (email não encontrado → 401, senha errada → 401 + audit, e-mail não verificado → 403 + audit, sucesso → tokens + audit), `register` (email duplicado → 409 + audit, dados válidos → cria usuário), `verifyEmail` (token inválido → 400 + audit, token válido → habilita usuário + audit sucesso), `resendVerificationEmail` (email não registrado → nada, já verificado → audit de falha, não verificado → envia e-mail + audit sucesso), `recoverPassword` (email não registrado → nada, email registrado → salva token no Redis + envia e-mail), `resetPassword` (token inválido → 400, token válido → atualiza senha + remove chave do Redis), `refreshToken` (token inválido → 401 + audit, token válido → novos tokens + audit sucesso)                                                                                                                                                                                                                                                                                                               |
| `UserService`             | 11     | `getAll` (delega ao repositório com filtro e pageable), `getActiveUsersCount` (delega contagem ao repositório), `update` (usuário não encontrado → 404, nome preenchido → atualiza, nome null → não altera), `getById` (não encontrado → 404, deletado + ADMIN → retorna, deletado + USER → 404), `changePassword` (senha errada → 400, senha correta → atualiza), `delete` (cascata stores/products/MinIO)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `CategoryService`         | 11     | `create` (loja não encontrada → 404, loja encontrada → cria), `getAll` (filtro null → 400, loja não encontrada → 404, filtro válido → página), `getById` (não encontrada → 404, encontrada → retorna), `update` (name null → não altera, name preenchido → altera), `delete` (não encontrada → 404, encontrada → deleta)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `SubscriptionService`     | 14     | `create` (usuário com assinatura ativa → 400, plano não encontrado → 404, dados válidos → cria assinatura e define role SUBSCRIBER), `getAll` (delega ao repositório com pageable), `getAllByUser` (delega ao repositório com userId e pageable), `getActiveByUser` (sem assinatura ativa → 404, encontrada → retorna), `getById` (não encontrada → 404, encontrada → retorna), `changePlan` (sem assinatura ativa → 400, mesmo plano → 400, plano diferente → desativa atual e cria nova), `cancelActiveSubscription` (sem ativa → 400, com ativa → cancela e reverte role para USER)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `OrderService`            | 20     | `create` (itens vazios → 400, endereço de outro usuário → 400, dados válidos → status COMPLETED verificado via ArgumentCaptor), `getAll` (delega ao repositório com pageable), `getAllByUser` (delega ao repositório com userId e pageable), `getAllByStore` (ADMIN bypassa ownership, USER loja não encontrada → 404, USER não-dono → 403, USER dono → retorna pedidos), `getMyOrderStores` (delega ao repositório), `getMyOrdersByStore` (delega ao repositório), `getSuccessfulSalesCount` (ADMIN bypassa + contagem, USER loja não encontrada → 404, USER não-dono → 403, USER dono → contagem de pedidos concluídos), `delete` (delega deleteById ao repositório), `getById` (404, não-dono com USER → 403, dono retorna DTO, ADMIN acessa pedido de qualquer usuário)                                                                                                                                                                                                                                                                                        |
...
| `UserAddressService`      | 8      | `create` (salva endereço e retorna DTO), `getAllByUser` (sem endereços → lista vazia, com endereços → DTOs mapeados), `update` (não encontrado → `IllegalArgumentException`, endereço de outro usuário → `IllegalArgumentException`, dono → atualiza e retorna DTO), `delete` (outro usuário → `IllegalArgumentException`, dono → soft delete)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `AnalyticsService`        | 10     | `getTotalRevenue(userId)` (null → ZERO, valor → encapsula em DTO), `getTotalRevenue()` global (null → ZERO, valor → retorna direto), `getUniqueCustomers(userId)` (retorna contagem de clientes únicos), `getTotalRevenueByStore` (owner com revenue null → ZERO, owner com revenue → encapsula), `verifyStoreAccess` via `getUniqueCustomersByStore` (ADMIN bypassa, loja não encontrada → 404, não-proprietário → 403)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `ProductController`          | 12     | `POST /api/products`, `GET /api/products`, `POST /api/products/store/products-by-ids`, `GET /api/products/store/{storeSlug}/product/{productSlug}`, `GET /api/products/{id}`, `GET /api/products/stores/stats/active-products`, `GET /api/products/store/{storeId}/stats/active-products`, `PATCH /api/products/{id}`, `DELETE /api/products/{id}` |
| `StoreController`            | 10     | `POST /api/stores`, `GET /api/stores`, `GET /api/stores/me`, `GET /api/stores/{id}`, `GET /api/stores/slug/{slug}`, `GET /api/stores/active/count`, `PATCH /api/stores/{id}`, `DELETE /api/stores/{id}`                                                                                                                                            |
| `SubscriptionController`     | 10     | `POST /api/subscriptions`, `GET /api/subscriptions`, `GET /api/subscriptions/user/{userId}`, `GET /api/subscriptions/user/me`, `GET /api/subscriptions/me/active`, `GET /api/subscriptions/{id}`, `PATCH /api/subscriptions/change-plan`, `PATCH /api/subscriptions/cancel`                                                                        |
...
| `CategoryController`         | 7      | `POST /api/categories`, `GET /api/categories`, `GET /api/categories/{id}`, `PATCH /api/categories/{id}`, `DELETE /api/categories/{id}`                                                                                                                                                                                                             |
| `SubscriptionPlanController` | 7      | `POST /api/subscription-plans`, `GET /api/subscription-plans`, `GET /api/subscription-plans/{id}`, `PATCH /api/subscription-plans/{id}`, `DELETE /api/subscription-plans/{id}`                                                                                                                                                                     |
| `FileController`             | 4      | `GET /api/files/{bucket}/**` (bucket inválido, erro MinIO, path traversal, arquivo válido)                                                                                                                                                                                                                                                         |
| `UserAddressController`      | 6      | `POST /api/addresses`, `GET /api/addresses`, `PUT /api/addresses/{id}`, `DELETE /api/addresses/{id}`                                                                                                                                                                                                                                               |
| `AuditLogController`         | 2      | `GET /api/audit-logs`, `GET /api/audit-logs/{id}`                                                                                                                                                                                                                                                                                                  |
| `FreightController`          | 1      | `GET /api/freight/{userAddressId}`                                                                                                                                                                                                                                                                                                                 |

**Total: 106 testes de controller**, cobrindo os seguintes padrões de requisição (não inclui `GlobalExceptionHandlerTest`, documentado na seção seguinte):
...
| Validação de entrada (`@Valid`)    | cenários 400 em `AuthController`, `CategoryController`, `OrderController`, `ProductController`, `StoreController`, `SubscriptionController`, `SubscriptionPlanController`, `UserAddressController`, `UserController` |

### GlobalExceptionHandler Testado

Testes com `@ControllerTest` sobre um `StubExceptionController` dedicado. Verificam que o `GlobalExceptionHandler` produz o envelope `{ status, code, message }` correto para cada tipo de exceção.

| Classe                       | Testes | Cenários cobertos                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GlobalExceptionHandlerTest` | 7      | `AccessDeniedException` → 403 com mensagem de acesso negado, `ResponseStatusException` → status e mensagem configurados, `MethodArgumentNotValidException` → 400 com lista de erros por campo, `HttpMessageNotReadableException` com enum inválido → 400 com valores permitidos, `HttpMessageNotReadableException` com JSON malformado → 400 com mensagem genérica, `RuntimeException` → 400 com "Erro na requisição", `Exception` genérica → 500 com "Erro interno do servidor" |

**Total: 7 testes de exception handler.**

### Specifications Testadas

Testes das classes `Specification` do JPA e da query builder do MongoDB. Duas estratégias distintas são utilizadas:

- **Mockito puro** (`@ExtendWith(MockitoExtension.class)`): verifica que o predicado correto é construído via `CriteriaBuilder` — sem banco de dados.
- **`@DataJpaTest` + H2** (`ProductSpecificationTest`): executa as specs contra um banco em memória real, verificando os resultados de consulta.

| Specification           | Testes | Estratégia      | Cenários cobertos                                                                                                                                                                                                                                                                                                                      |
| ----------------------- | ------ | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AuditLogSpecification` | 13     | Unitário (puro) | `startDate` null → epoch, `endDate` null → 9999-12-31, datas fornecidas sobrepõem defaults; `userId` null/vazio → não adicionado, preenchido → adicionado; `action` null/vazio → não adicionado, string válida (case-insensitive) → enum mapeado, string inválida → `IllegalArgumentException`; ordenação → descending por `timestamp` |
| `CategorySpecification` | 2      | Mockito         | `store(id)` → predicado `equal` em `store.id`; `name(str)` → predicado `LIKE` case-insensitive com `%`                                                                                                                                                                                                                                 |
| `ProductSpecification`  | 11     | `@DataJpaTest`  | `store(id)` → retorna apenas produtos da loja; `category(id)` → filtra por categoria; `name(str)` → LIKE case-insensitive e substring parcial; `minPrice` → `>=`; `maxPrice` → `<=`; `lowStock(threshold)` → `<=`; `status` ACTIVE/DELETED/ALL                                                                                         |
| `StoreSpecification`    | 3      | Mockito         | `user(id)` → predicado `equal` em `user.id`; `active()` → `isNull(deletedAt)`; `deleted()` → `isNotNull(deletedAt)`                                                                                                                                                                                                                    |
| `UserSpecification`     | 5      | Mockito         | `name(str)` → LIKE case-insensitive; `email(str)` → LIKE case-insensitive; `role(role)` → predicado `equal`; `active()` → `isNull(deletedAt)`; `deleted()` → `isNotNull(deletedAt)`                                                                                                                                                    |

**Total: 34 testes de specification.**

### Segurança Testada

Testes dos componentes de segurança Spring: o filtro JWT e o entry point de autenticação. Ambos usam `@ExtendWith(MockitoExtension.class)` sem contexto Spring.

| Componente                    | Testes | Cenários cobertos                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `JwtAuthenticationEntryPoint` | 2      | `commence` → 401 com `Content-Type: application/json`; corpo JSON contém `status:"error"`, `code:401` e mensagem de token inválido                                                                                                                                                                                                                                                                                                                                                                             |
| `JwtAuthenticationFilter`     | 9      | Sem header e sem cookie → chain continua sem autenticação; Bearer válido + usuário encontrado → autentica no `SecurityContext`; cookie `token` válido + usuário encontrado → autentica; token inválido → chain continua sem autenticação; token válido mas usuário não encontrado no DB → 401 sem chamar chain; header não-Bearer → não valida token; cookies sem nome `token` → não valida token; role do usuário mapeada como `GrantedAuthority`; Bearer tem precedência sobre cookie quando ambos presentes |

**Total: 11 testes de segurança.**

### Utilitários Testados

Testes unitários puros das classes utilitárias, sem dependências externas.

| Utilitário   | Testes | Cenários cobertos                                                                                                                                                                                                                                                                                                 |
| ------------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RandomUtil` | 8      | `chance(0)` → sempre false; `chance(100)` → sempre true; valor negativo → false; valor > 100 → true; `randomPastDate(0)` → data de hoje; `randomPastDate(n)` → dentro do intervalo; `randomDeletion(0%)` → `isDeleted=false`, `deletedAt=null`; `randomDeletion(100%)` → `isDeleted=true`, `deletedAt` preenchido |
| `SlugUtil`   | 8      | Texto simples → slug minúsculo com hífens; acentos → normalizados sem acento; caracteres especiais → removidos; null → string vazia; string em branco → string vazia; hífens nas bordas → removidos; números preservados; múltiplos espaços → único hífen                                                         |

**Total: 16 testes de utilitários.**
...
**Rodar todos os testes de specification:**

```bash
./mvnw test -Dtest="AuditLogSpecificationTest,CategorySpecificationTest,ProductSpecificationTest,StoreSpecificationTest,UserSpecificationTest"
```

**Rodar todos os testes de segurança e utilitários:**

```bash
./mvnw test -Dtest="JwtAuthenticationEntryPointTest,JwtAuthenticationFilterTest,RandomUtilTest,SlugUtilTest,GlobalExceptionHandlerTest"
```
